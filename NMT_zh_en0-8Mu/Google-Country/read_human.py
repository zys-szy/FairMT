...
with open("Com_ALL.txt", "r") as f:
    ...
    for i in range(0, len(lines), 15):
        ...
        if vote >= 3:
            ground_new[9][lines[i + 9].strip()] = False
        else:
            ground_new[9][lines[i + 9].strip()] = True
...
index = ["LCS-O", "ED-O", "TF-O", "BLEU-O", "LCS", "ED", "TF", "BLEU", "BERT", "Vote"]
out_list = [[] for i in range(10)]
dic = {}
dic[True] = "P"
dic[False] = "N"
#dic[tuple([False])] = "TN"
...
count = [collections.Counter() for i in range(10)]
#sum = [0] * 8
