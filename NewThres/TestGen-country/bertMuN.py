...
#    print (mg2.getMutants())
    ...
    dic = {}
    count = 0
    if len(mg2.getMutants()) == 0:
        continue
    for i in range(len(mg2.getMutants()[0])):
        sen = mg2.mutants[0][i]
        ct = mg2.countries[0][i]
        if ct not in dic:
            dic[ct] = [sen]
        else:
            dic[ct].append(sen)
...
#        break
        ...
        if n not in usedd:
            ...
            continue
        print ("Output: ")
        print ("Gen:\tmale" + llll[index1] + "\t" + n[0])
        print ("Gen:\tmale" + llll[index2] + "\t" + n[1])
        countct += 1
        f.write("Gen:\tmale" + llll[index1] + "\t" + detk(n[0]).strip() + "\n")
        f.write("Gen:\tmale" + llll[index2] + "\t" + tk(n[1]).strip() + "\n")
        fline.write(str(i) + "\n")
        count += 1
    #continue
    #tag = nlp.pos_tag(line)
    ...
    dic = {}
    count = 0
    for i in range(len(mg2.getMutants()[1])):
        sen = mg2.mutants[1][i]
        ct = mg2.countries[1][i]
        if ct not in dic:
            dic[ct] = [sen]
        else:
            dic[ct].append(sen)
...
#        break
        ...
        if n not in usedd:
            ...
            continue
        print ("Output: ")
        print ("Gen:\tfemale" + llll[index1] + "\t" + n[0])
        print ("Gen:\tfemale" + llll[index2] + "\t" + n[1])
        countct += 1
        f.write("Gen:\tfemale" + llll[index1] + "\t" + detk(n[0]).strip() + "\n")
        f.write("Gen:\tfemale" + llll[index2] + "\t" + tk(n[1]).strip() + "\n")
        fline.write(str(i) + "\n")
        count += 1
f.close()
fline.close()
