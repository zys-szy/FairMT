...
#            print(template)
#            print (placeholder)
#            template = self.replaceGenderPronounPlaceholderIntoFemale(template, tempplh, gender)
#            print (template)
#            exit()
        mmutants, mnames, mcountries = self.generateMaleMutant(template, plhs, gender)        
#            elif gender == "female" :
        fmutants, fnames, fcountries = self.generateFemaleMutant(template,  plhs, gender)        
    
        
        return templates, [mmutants, fmutants], [mnames, fnames], [mcountries, fcountries]
#            if len(mutants) > 0 :
...
#                self.examples.append(mutants[0])
    ...
    def generateMaleMutant(self, template, plhs, gender) :
        pronoun_placeholders = getPronounPlaceholders(plhs)
        mutants = []
        ...
        non_pronoun_placeholders = plhs.difference(pronoun_placeholders)

        for name, country in zip(names, countries) :
            mutant = template.replace(tag(NAME), name.title())
            mutants.append(mutant)
        mutants = self.replaceGenderAssociatedWordPlaceholder(mutants, getMasculineGenderAssociatedWord())

        return mutants, names, countries
                
    def generateFemaleMutant(self, template, plhs, gender) :
        ...
        mutants = self.replaceGenderAssociatedWordPlaceholder(mutants, getFeminineGenderAssociatedWord())
